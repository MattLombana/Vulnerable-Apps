...
def admin():
    if current_user.is_admin():
        messages = db.get_admin_messages()
        args = {'active': 'admin', 'messages': messages}
...
def messageadmin():
    if request.method == 'GET':
        args = {'active': 'messageadmin'}
        return render_template('messageadmin.html', args=args)
    else:
        db.insert_admin_message(request.form['message'])
...
def elevate():
    ...
    if request.method == 'GET':
        args = {'active': 'elevate'}
        return render_template('elevate.html', args=args)
    else:
        db.mark_admin(request.form['username'])
        flash('User {} has been promoted to admin'.format(request.form['username']))
        return redirect(url_for('index'))
